...
class NGramMarginal : public NGramMutableModel {
 public:
  ...
  NGramMarginal(StdMutableFst *infst, Label backoff_label = 0,
  ...
  bool MarginalizeNGramModel(vector<double> *weights, int iter, int tot) {
    ...
    if (!CheckNormalization())  // model should be normalized
      LOG(FATAL) << "NGramMarginal: Marginalized model not fully normalized";
  ...
  double SaneDenominator(double total_wbo, double found_sum, 
			 double log_prob) {
    double minval = fmax(-33, log_prob), ret = (found_sum > total_wbo) ? 
      -NegLogDiff(total_wbo, found_sum) : minval;
